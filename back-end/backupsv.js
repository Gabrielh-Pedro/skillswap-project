...
mongoose.connect('mongodb://localhost:27017/originalskillswap')
  ...
  app.post('/api/register', upload.single('photo'), async (req, res) => {
    try {
      ...
      const token = jwt.sign({ id: user._id }, 'secrettoken', { expiresIn: '5h' });
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: user._id }, 'secrettoken', { expiresIn: '5h' });
...
app.post('/updatePrivacy', upload.single('photo'), async (req, res) => {
...
app.post('/match/confirm/:matchId', async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const decoded = jwt.verify(token, 'secrettoken');
  const userId = decoded.userId;
...
app.post('/match/:postId', async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const decoded = jwt.verify(token, 'secrettoken');
  const userId = decoded.userId;
...
app.get('/api/swaps/received-matches', async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const decoded = jwt.verify(token, 'secrettoken');
  const userId = decoded.userId;
...
app.get('/api/matches/confirmed', async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const decoded = jwt.verify(token, 'secrettoken');
  const userId = decoded.userId;
...
app.get('/check-matched/:postId', async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const decoded = jwt.verify(token, 'secrettoken');
  const userId = decoded.userId;
  const { postId } = req.params;
